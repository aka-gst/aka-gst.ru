...
for (const [экран, w, h, mobile] of [
  ...
  проверить('при загрузке не раскрыт ни один сборник', сперва.length === 0, сперва.join(', ') || 'все закрыты');

  // Экран не должен ехать сам. Мерить надо scrollY до и после нажатия, а
  // не искать в коде слова: владелец жалуется на движение, а не на текст.
  //
  // Сначала раскрываем сборник — со свёрнутыми страница короче экрана и
  // прокручивать нечего, а замер, который не может не сойтись, ничего не
  // стережёт (правило 7и). Затем уезжаем вниз и жмём ДРУГУЮ карточку:
  // именно тут прежний код подвозил её к верху.
  await из(`document.querySelectorAll('.bcard')[0].click(); 1`);
  await sleep(400);
  await из(`scrollTo(0, 420); 1`);
  await sleep(300);
  const до = await из('Math.round(scrollY)');
  проверить('странице есть куда ехать (иначе замер пустой)', до > 100, `scrollY ${до}`);
  await из(`document.querySelectorAll('.bcard')[1].click(); 1`);
  await sleep(900);
  const после = await из('Math.round(scrollY)');
  проверить('страница не везёт экран сама', до === после, `было ${до}, стало ${после}`);
  проверить(
    'плавной прокрутки нет',
    (await из(`getComputedStyle(document.documentElement).scrollBehavior`)) === 'auto'
  );

  const второй = JSON.parse(await из(`JSON.stringify(${открытые})`));
  проверить('нажатие на карточку раскрывает её', второй.length === 1, второй.join(', ') || 'ничего');
