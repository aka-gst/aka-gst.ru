...
const собрать = ({ game, кадров, секунд, лог, режим, сценарно }) => {
  ...
  // Неподвижность меряем движением, а не весом файла. Вес обманывает: у
  // «Деревни» плоская векторная графика жмётся в 9 КБ при живой картинке, и
  // порог по весу дважды обозвал пустыми нормальные петли. Считаем разницу
  // между соседними кадрами и смотрим, сколько она весит на секунду.
  //
  // Отрицательный контроль (правило 7л): заведомо неподвижная петля из
  // одного повторённого кадра даёт 1 КБ/с, а девять живых — от 4 до 24.
  // Порог 3 КБ/с стоит между ними с тройным запасом в обе стороны.
  const разн = `${OUT}.raznica-${game.id}.mp4`;
  spawnSync('ffmpeg', ['-y', '-hide_banner', '-loglevel', 'error', '-i', цель,
    '-vf', 'tblend=all_mode=difference', '-c:v', 'libx264', '-crf', '30',
    '-preset', 'ultrafast', '-an', разн]);
  const движение = existsSync(разн) ? statSync(разн).size / 1024 / Math.max(0.1, длит) : 0;
  rmSync(разн, { force: true });
  const пустая = движение < 3;
  const стояла = кадров < 20;
  const тяжёлый = кб > ПОТОЛОК_КБ;
  const беды = [
    стояла && 'ИГРА СТОЯЛА',
    пустая && `НЕ ДВИЖЕТСЯ: ${движение.toFixed(1)} КБ/с разницы`,
    тяжёлый && `ТЯЖЕЛО: ${кб} КБ > ${ПОТОЛОК_КБ}`,
  ].filter(Boolean);
  console.log(`${game.id.padEnd(12)} ${режим} ${лог.join(' ')} · кадров ${String(кадров).padStart(3)} за ${секунд.toFixed(1)}с · ролик ${длит.toFixed(1)}с · ${кб} КБ · движение ${движение.toFixed(1)} КБ/с${беды.length ? '  ← ' + беды.join(', ') : ''}`);
...
const run = async () => {
  ...
  for (const game of PLAN) {
    ...
    const план = { ...(цельЗаписи.during || { seconds: 5 }) };
    ...
    // Игра может сама сказать, как её снимать. «Деревня» возвращает из
    // showcase метаданные синхронно: {intended: 2600, capture: 'frames'}.
    // Это лучше, чем держать такое знание у меня в плане: пока режим
    // прописан здесь, он живёт в переписке и устаревает молча — сцена
    // переедет с вёрстки на холст, а записывалка будет снимать по-старому.
    if (план.спроси) {
      const о = await send('Runtime.evaluate', { expression: план.спроси, returnByValue: true })
        .catch(() => null);
      const м = о?.result?.value;
      if (м && typeof м === 'object') {
        if (м.capture === 'frames') план.покадрово = true;
        if (м.capture === 'screencast') план.покадрово = false;
        // Хвост в треть секунды: намеренная длительность — это про содержание,
        // а последний кадр должен успеть дожить, а не обрубиться на движении.
        if (Number(м.intended) > 0) план.seconds = м.intended / 1000 + 0.3;
        лог.push(`игра просит: ${м.capture || 'как есть'}, ${м.intended ?? '?'} мс`);
      }
    }


    // Покадрово по часам, а не трансляцией. Трансляция шлёт кадр, когда
    // композитор что-то закоммитил; у «Неон Линий» доска из элементов
    // страницы меняется мелко, и за три секунды пришёл ОДИН кадр. Снимок же
    // сам вынуждает страницу отрисоваться, поэтому здесь берём их по
    // расписанию — медленнее, но не зависит от того, считает ли браузер
    // изменения достойными кадра.
    if (план.покадрово) {
      const конец = Date.now() + (план.seconds || 3) * 1000;
      const шагМс = 1000 / (план.fps || 20);
      const началоП = Date.now();
      while (Date.now() < конец) {
        const т = Date.now();
        const к = await send('Page.captureScreenshot', { format: 'jpeg', quality: 82 });
        writeFileSync(`${TMP}f${String((n += 1)).padStart(4, '0')}.jpg`, Buffer.from(к.data, 'base64'));
        const ждать = шагМс - (Date.now() - т);
        if (ждать > 0) await sleep(ждать);
      }
      лог.push('покадрово');
      if (собрать({ game, кадров: n, секунд: (Date.now() - началоП) / 1000, лог, режим: 'покадр' }).плохо) плохие.push(game.id);
      continue;
    }
