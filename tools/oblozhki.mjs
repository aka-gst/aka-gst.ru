...
const полка = (исток, цель) => {
  // Качество 78 → 68. Раньше обложка сборника была одна на первом экране,
  // теперь список свёрнут и все три стоят рядом: 301 КБ разом на критическом
  // пути подняли LCP с 1.74 до 2.64 с. На высоте показа в 300 пикселей
  // разницы между 78 и 68 не видно, а на пути она видна.
  sips(['--resampleHeight', String(ПОЛКА), '-s', 'format', 'jpeg', '-s', 'formatOptions', '68',
    исток, '--out', цель]);
};
