...
const подмести = (порт) => {
  try {
    // НЕ pgrep/pkill. На macOS `pgrep -f` сопоставляет ОБРЕЗАННУЮ командную
    // строку, а у дочерних процессов Chrome флаг порта стоит примерно на
    // 505-м символе строки длиной 1620 — и pgrep их не видит вовсе.
    // Замерено 5 сентября: `pgrep -f 'remote-debugging-port=9411'` даёт 0,
    // `ps axo command | grep` — 3, и это живые сироты уже закрытого прогона.
    // Из-за этой слепоты я утром доложил, что chrome.kill() убирает всё
    // подчистую. Неправда: дети переживают его, и Глаза-Уши были правы.
    // `-axww` обязателен: без него `ps` обрезает командную строку по ширине
    // терминала и флаг порта (505-й символ) в неё не попадает.
    // Скобка вокруг первой цифры — чтобы сам ищущий grep не попал в выборку:
    // его командная строка содержит скобки и образцу не соответствует.
    const п = String(порт);
    const образец = `remote-debugging-port=[${п[0]}]${п.slice(1)}`;
    const хвосты = execSync(`ps -axww -o pid,command | grep '${образец}' | awk '{print $1}' || true`)
      .toString().trim().split('\n').filter(Boolean);
    if (!хвосты.length) return 0;
    execSync(`kill -9 ${хвосты.join(' ')} 2>/dev/null || true`);
