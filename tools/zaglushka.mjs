#!/usr/bin/env node
// ВАЖНО про clip-psy-admin: у этой карточки расхождение заглушка↔кадр
// БОЛЬШОЕ НАМЕРЕННО, и чинить его не надо. Петля показывает, как форму
// заполняют: пустое поле → печатается вопрос → приходит ответ. Её нулевой
// кадр — пустая форма и 60% пустого фона, и как неподвижная картинка он
// хуже всего. Заглушка поэтому равна ПОСЛЕДНЕМУ кадру, то есть готовому
// ответу: человек почти всегда видит именно заглушку, а движение — только
// при наведении. Решение Глаз-Ушей от 1 сентября 2026, проверено глазами:
// содержание последнего кадра и заглушки совпадает, расходится лишь высота
// на 38 CSS.
...
// Ноль роликов — это не «все хороши», а «мерить было нечего». Без этого
// порога цикл ниже не выполняется ни разу, инструмент молчит и выходит с
// нулём, и «проверено» означает «не проверено». Подсказка сессии «Боты»
// 31 августа 2026: у них обёртка звала несуществующий на macOS `timeout`,
// три прогона не состоялись, и пустой вывод выглядел как «проверки
// бесполезны». Молчание меры одинаково убедительно и как зелёное, и как
// красное, и врёт в обе стороны.
if (сколько === 0) {
  console.log('  МЕРИТЬ НЕЧЕГО: ни одного .shot .gclip на странице');
  send.закрыть();
  chrome.kill();
  process.exit(1);
}
...
// Инструмент до сих пор всегда выходил нулём, что бы ни намерил. Числа он
// печатает для человека, но «страница не открылась» и «стык идеален»
// должны различаться и для того, кто зовёт его из скрипта.
process.exit(0);
