...
QA Quest выполняет в браузере. Сейчас они в репозитории.

Выкладка от этого **не зависит**: `QA Quest/tools/deploy.sh` докачивает
`vendor/pyodide`, если его нет в дереве, и после заливки сверяет sha256 всех
пяти файлов на живом домене. Проверено. Поэтому удаление их из git ничего не
ломает — но и держать их здесь не обязательно.

Останутся они в репозитории или переедут в скрипт — решает владелец; пока
решение не принято, не трогайте их «на всякий случай».
