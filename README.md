...
### qa-quest/vendor держится в git намеренно

В `qa-quest/vendor/pyodide/` лежит около 13 МБ бинарников — это Python,
который QA Quest выполняет в браузере. Они в репозитории специально: без них
страница молча уйдёт за средой исполнения на чужой CDN, а выкладка перестанет
быть самодостаточной. Не удаляйте их «на всякий случай» — сначала правьте
выкладку. Решение принято сессией QA Quest, она же сверяет sha256 живого
`pyodide.mjs` после заливки.
